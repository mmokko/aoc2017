CHECKSUM = '''798	1976	1866	1862	559	1797	1129	747	85	1108	104	2000	248	131	87	95
...
class CheckSumCalculator(object):
    ...
    def _get_rows(self):
        for row in self.__cs.split('\n'):
            yield [int(val) for val in row.split()]

    def calculate_diff(self):
        total = 0
        int_rows = self._get_rows()
        for row in int_rows:
            total += max(row) - min(row)
        return total

    @staticmethod
    def find_div_sum(row):
        sorted_row = sorted(row)
        for idx, val in enumerate(sorted_row):
            for i in range(idx + 1, len(sorted_row)):
                if sorted_row[i] % val == 0:
                    return sorted_row[i] / val
        return 0
    ...
    def calculate_div(self):
        ...
        int_rows = self._get_rows()
        for row in int_rows:
            total += CheckSumCalculator.find_div_sum(row)
        return total


def main():
    csc = CheckSumCalculator(CHECKSUM)
    print(csc.calculate_diff())
    print(csc.calculate_div())
